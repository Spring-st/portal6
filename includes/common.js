...
function checkCenturyDate(dateValue) {	
	...
	function cascadeSiteDepartment(value,select){
			var selects = document.getElementById(select);
			selects.length = 0;
		 $.ajax({         
                type:"POST", //请求方式        
                url:"cascadeSiteDepartment.do", //请求路径        
                cache: false,           
                data:"siteid="+value,  //传参        
                dataType: 'json',   //返回值类型        
                success:function(json){
					 for (i = 0; i < json.length; i++){
						 var obj;      
						  obj=document.createElement("option");     
						  obj.value=json[i].property;      
						  obj.text=json[i].labelProperty;   
						  selects.add(obj);
					 } 
			    } 
            }        
