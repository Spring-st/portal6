...
<script type="text/javascript">
	var sign=false;
<%--		function poPuInStorageMaterialByBox(){ 
			  var r = false;
  				...
  				// sign=true;
					...
					.showModalDialog(
							'showDialog.do?title=purchaseOrderRqcDetermine.title&produceRejectedMaterialBoxByReasons.do?arrays='
									+ str + "&type=2&ids="+ids, null,
							'dialogWidth:500px;dialogHeight:300px;status:no;help:no;scroll:no');
				if (v) {
					window.location.href = 'listpurchaseOrderPutInStorageMaterialBoxList.do';
				}
 				//$.ajax({
				//	type : "POST", //请求方式        
				//	url : "poPutInStorageMaterialBoxByAjax.do", //请求路径        
				//	cache : false,
				//	data : "array=" + str,      
				//	dataType : 'json', //返回值类型        
...
function poPuInStorageMaterialByBox(){
	...
	if(!sign){
		...
		if (r) {
			window.location.href = 'updateProduceRejectedMaterialBox.do?ids='+str+'&type=2';
		}else{
			sign=false;
		}
	}else{
...
<html:form action="/listpurchaseOrderPutInStorageMaterialBoxList.do" styleId="formId">
...
<page:form action="/listpurchaseOrderPutInStorageMaterialBoxList.do" method="post">
	<table class="data">
		<thead>
