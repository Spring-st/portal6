				 ...
				 if(el[i].id!="checkAll"){
      			...
      			r = confirm("确认扫描上架吗？");
       		    if(r){
					...
					'showDialog.do?title=purchaseOrderPutInStorage.title&purchaseOrderRqcDetermine.do?arrays='+str+"&type=" + 4, 
					null, 'dialogWidth:500px;dialogHeight:350px;status:no;help:no;scroll:no');
				if (v) {
					 window.location.href = 'purchaseOrderPutInStorageBoxList.do';
				};
       		 }
		}
}
function poBlpInStorage(type){ 
			if(type!=""){
			  var r = false;
			  var el = document.getElementsByName('ids');
		 	  var len = el.length;   
  			 ...
  			 if((el[i].type=="checkbox") && (el[i].checked== true)){
				 if(el[i].id!="checkAll"){
      			...
      			r = confirm("确认帐外不良品入库吗？");
       		    if(r){
       			  v = window.showModalDialog(
					'showDialog.do?title=purchaseOrderPutInStorage.title&purchaseOrderRqcDetermine.do?arrays='+str+"&type=" + 5, 
					null, 'dialogWidth:500px;dialogHeight:350px;status:no;help:no;scroll:no');
				if (v) {
					 window.location.href = 'purchaseOrderPutInStorageBoxList.do';
				};
...
<html:form action="/purchaseOrderPutInStorageBoxList.do" styleId="formId">
	...
	<input type="hidden" name="fields" value="pb.po_date" desc="到货时间"/>
...
<page:form action="/purchaseOrderPutInStorageBoxList.do" method="post">
	<table class="data">
		<thead>
