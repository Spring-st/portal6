...
function printSPList(psoId) {
		...
		window.location.href="printSalesWorkShipOrder.do?id="+psoId;
	}
function SPListClose(psoId) {
		
		window.location.href="SalesWorkShipOrderClose.do?id="+psoId;
	}

function goTo(id){
		var url = "listSalesWorkorder.do";
		window.location.href = url;
	}
function SPListOpen(psoId) {
		$.ajax({         
        	type:"POST", //请求方式     
        	//url:"salesShipOrderByBoxAJAX.do", //请求路径  
            url:"salesWorkShipOrderOpenAJAX.do", //请求路径        
            cache: false,   
            data:"id="+psoId,  //传参        
            dataType: 'json',   //返回值类型        
            async:false,
            success:function(json){
            	if(json =="false"){
               		alert("发货单批次已同步，不能再次打开!");
               		return false;
                }else{
                	//window.location.href="listSalesWorkorder.do";
                	window.location.href="viewSalesPreShipOrder.do?id="+psoId;
...
<html:form action="/viewSalesPreShipOrder" method="post">
	<table width="100%">
	<tr>
...
<page:form action="/viewSalesPreShipOrder">	
	...
	<table class="data" width="100%" style="display: inline;">
				...
				<c:set var="X_OBJECT"  scope="request" value="X_OBJECT"/>
					<tr align="center">
						...
						<td><fmt:formatNumber value="${X_OBJECT.count}" maxFractionDigits="0" /></td>
						<td>
						...
						<td>${X_OBJECT.status1.chnShortDescription}</td>
				</tr>
				</c:forEach>
				</tbody>
	</table>
	<table width="100%">
		...
		<tr>
			...
			<c:if test="${x_salesShipOrder.status=='1'}">
				<input type="button" value="关闭发货单" onclick="SPListClose('${x_salesShipOrder.id}')"/>
			</c:if>
			<c:if test="${x_salesShipOrder.status=='2' && x_salesShipOrder.shPrint ne '0'}">
				<input type="button" value="打开发货单" onclick="SPListOpen('${x_salesShipOrder.id}')"/>
				<input type="button" value="打印发货单" onclick="printSPList('${x_salesShipOrder.id}')"/>
			</c:if>
			<c:if test="${x_salesShipOrder.status=='2' && x_salesShipOrder.shPrint eq '0'}">
				<input type="button" value="打印发货单" onclick="printSPList('${x_salesShipOrder.id}')"/>
			</c:if>
