    ...
    function printCodeMax(){
	    ...
	    var el = document.getElementsByName('ids');  
  		...
  		for(var i=0; i<len; i++){   
			...
			window.location.href = "printCodePurchaseOrderInspectionPendingItemProduct.do?str="+str.toString();
	}
	 function printCodeMin(){
	    ...
	    var el = document.getElementsByName('ids');  
  		...
  		for(var i=0; i<len; i++){   
			...
			//window.location.href = "printCodePurchaseOrderInspectionPendingItem.do?supplierBatch="+supplierBatch+"&str="+str.toString();
			window.location.href = "printCodePurchaseOrderInspectionPendingItemProductMin.do?str="+str.toString();
	}
	  function printCodeZhong(){
	    ...
	    var el = document.getElementsByName('ids');  
  		...
  		for(var i=0; i<len; i++){   
			...
			//window.location.href = "printCodePurchaseOrderInspectionPendingItem.do?supplierBatch="+supplierBatch+"&str="+str.toString();
			window.location.href = "printCodePurchaseOrderInspectionPendingItemProductZhong.do?str="+str.toString();
	}
	  function qrCodeMin(){
			//var supplierBatch = document.getElementById("supplierBatch").value;
			//if(supplierBatch==""){
		    ...
		    var el = document.getElementsByName('ids');  
	  		...
	  		for(var i=0; i<len; i++){   
				...
				//window.location.href = "printCodePurchaseOrderInspectionPendingItem.do?supplierBatch="+supplierBatch+"&str="+str.toString();
				window.location.href = "printCodePurchaseOrderItemqrCodeMin.do?str="+str.toString();
		}
		  function qrCodeZhong(){
			//var supplierBatch = document.getElementById("supplierBatch").value;
			//if(supplierBatch==""){
			//	alert("请输入供应商批次!");
			//	return false;
		    ...
		    var el = document.getElementsByName('ids');  
	  		...
	  		for(var i=0; i<len; i++){   
				...
				//window.location.href = "printCodePurchaseOrderInspectionPendingItem.do?supplierBatch="+supplierBatch+"&str="+str.toString();
				window.location.href = "printCodePurchaseOrderItemqrCodeZhong.do?str="+str.toString();
		}
		  function qrCodeMax(){
				//var supplierBatch = document.getElementById("supplierBatch").value;
				//if(supplierBatch==""){
			    ...
			    var el = document.getElementsByName('ids');  
		  		...
		  		for(var i=0; i<len; i++){   
					...
					//window.location.href = "printCodePurchaseOrderInspectionPendingItem.do?supplierBatch="+supplierBatch+"&str="+str.toString();
					window.location.href = "printCodePurchaseOrderItemqrCode.do?str="+str.toString();
			}
   function uncheckAll() {  
        var el = document.getElementsByName('ids');   
  		var len = el.length;   
...
//-->
	function ret(obj){
		//alert(obj);
		window.location.href="viewPortalShipOrderTwoProduct.do?id="+obj;
	}
</script>
 <html:form action="/printPortalShipOrderPartCodeListProduct"  method="post">
 </html:form>
 <page:form action="/printPortalShipOrderPartCodeListProduct"  method="post">
<table width="100%">
<tr><td>
...
<tr><td>
         ...
         --%><c:if test="${sessionScope.LOGIN_USER.locale=='zh'}"><input type="button" value="打印二维码小标签"  onclick="qrCodeMin()"/></c:if>
 		<c:if test="${sessionScope.LOGIN_USER.locale=='zh'}"><input type="button" value="打印二维码中标签"  onclick="qrCodeZhong()"/></c:if>
         <c:if test="${sessionScope.LOGIN_USER.locale=='zh'}"><input type="button" value="打印二维码大标签"  onclick="qrCodeMax()"/></c:if>
