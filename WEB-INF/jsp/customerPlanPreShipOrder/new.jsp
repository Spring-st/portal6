...
function selectCustomerPlanList(){
	...
	v = window.showModalDialog(
			'showDialog.do?title=site.purchase.title&selectCustomerPlan.do?customerPlamId='+selectSoipitemId, 
		...
		if (v) {
			...
			for(i = 0;i<resultList.length; i++){
		        ...
		        trString += "<td align='center'>"+"<input type=\"hidden\" name=\"planNumbers\" value=\""+(v['planNumbers'])+"\" />"+"<input type=\"hidden\" name=\"soItemIds\" value=\""+(v['customerPlanId'])+"\" />"+v['planNumbers']+"</td>";
		        trString += "<td>"+v['line']+"</td>";
		        trString += "<td>"+v['customerCode']+"</td>";
		        trString += "<td>"+v['customerAddress']+"</td>";
		        trString += "<td>"+v['wmspartId']+"</td>";
//		        trString += "<td align='left'>"+"<input type=\"hidden\" name=\"itemNumbers\" value=\""+(v['itemNumberId'])+"\" />"+v['itemNumberId']+"</td>";
		        trString += "<td>"+v['wmspartDpiNo']+"</td>";
		         trString += "<td>"+v['wmspartoldCode']+"</td>";
		        trString += "<td>"+v['wmspartDescribe1']+"</td>";
		        trString += "<td>"+v['wmspartUnit']+"</td>";
		        trString += "<td>"+v['shipmentDate']+"</td>";
...
function createCustomerPlanPreShipOrder(){
	...
	$.ajax({         
		type:"POST", //请求方式        
		url:"createCustomerPlanPreShipOrderAJAX.do", //请求路径     
		cache: false,   
       	data:"soItemIds="+str+"&deliveryNumbers="+deliveryNumbers+"&remarks="+remarks+"&receivingDate="+receivingDate,  //传参        
        dataType: 'json',   //返回值类型        
        success:function(date){ 
        	if(date.result == "1"){
        		...
        		return false;
              		...
              		window.location.href = "listCustomerPlanPreshiporder.do";
            	}
        	}
		});
...
<html:form action="/createCustomerPlanPreShipOrder" method="post">
...
<h3 style="color:blue"><bean:message key="purchaseOrderReceipts.item"/></h3>
	...
	<table width="100%">
		...
		<tr>
			...
			<input type="button" value="生成发货单" id="scfhd" onclick="createCustomerPlanPreShipOrder()"/>
			</td>
		</tr>
	</table>
</html:form>
