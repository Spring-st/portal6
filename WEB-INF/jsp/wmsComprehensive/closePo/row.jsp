...
<tr id="r${X_OBJECT.id}" align="center">
	<td>
	<c:if test="${X_OBJECT.status == '1'}"><input type="checkbox" name="ids" value="${X_OBJECT.id}" onclick="productbox_click();"/></c:if>
	<c:if test="${X_OBJECT.status == '2'}"><input type="checkbox" disabled="disabled"/></c:if>
	</td>
