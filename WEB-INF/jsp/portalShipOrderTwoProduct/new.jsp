...
function selectWmsPart(){
		...
		var r = window.showModalDialog(
			'showDialog.do?title=all.select&selectWmsPartShipOrder.do?portalShipJitPartId='+partStr, 
		...
		if (r) { 
			...
			for(i = 0;i<resultList.length; i++){
		        ...
		        trString += "<td>"+v['productSpecifications']+"</td>";
...
<html:form action="/createPortalShipJitPart" onsubmit="return validateForm();">
<input type="hidden" id="createType" name="createType" value="JIT"/>
...
<h3 style="color:blue"><bean:message key="purchaseOrderPart.item"/></h3>
	...
	<table width="100%">
		...
		<tr>
			...
			<input  id="submita" type="submit" value="生成发货单" onclick="onclicks();"/>
			</td>
		</tr>
	</table>
</html:form>
