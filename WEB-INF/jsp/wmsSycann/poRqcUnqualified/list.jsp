  			 ...
  			 if((el[i].type=="checkbox") && (el[i].checked== true)){
				 if(el[i].id!="checkAll"){
  		        ...
  		        if(count==0){
  			      ...
  			      r=confirm("已选中批次确认要扫描的条码编号");
  			  if(r){
  				  	  v = window.showModalDialog(
			'showDialog.do?title=userRole.new.title&newDefeScanning.do?array='+str, 
			null, 'dialogWidth:550px;dialogHeight:380px;status:no;help:no;scroll:no');
			if (v) {
			window.location.href = 'listPurchaseOrderRQCUnqualified.do'; 
		}
...
function updateRqc(){
           ...
           if(r){
  			 window.location.href = 'insertDefeScanningQua.do?array='+str;
			}
           
          

}
function updateNotRqc(){
			 var el = document.getElementsByName('ids');
		 	  var len = el.length;   
  		 	  var str="";
  			  var count=0;
           ...
           if(r){
  			 window.location.href = 'insertDefeScanningNOtQua.do?array='+str;
			}
...
<html:form action="/listPurchaseOrderRQCUnqualified.do" styleId="formId">
...
<page:form action="/listPurchaseOrderRQCUnqualified.do" method="post">
	<table class="data">
		<thead>
