 	...
 	//	alert("aaa");
		...
		for(i=0;i<check.length;i++){
			if(check[i].checked == true){
				...
				map['itemNumberDpi'] = document.getElementById('td_itemNumberDpi'+id).innerHTML;
...
<html:form action="listPurchaseItemTwoWrong" >
...
<page:form action="listPurchaseItemTwoWrong.do" method="post">
		...
		<tbody id="datatable">
			<logic:iterate id="X_OBJECT" name="X_RESULTLIST">
				...
				<tr align="center">
					...
					<td > <input type="hidden" id="qty_box${X_OBJECT.id}" value="${X_OBJECT.qty_std}"/><input size="10" type="text" <c:if test="${X_OBJECT.checked=='checked'}">disabled="disabled"</c:if> id='td_fahuoNumber${X_OBJECT.id}'/> <input type="hidden" id="vd_promo${X_OBJECT.id}" value="${X_OBJECT.vd_promo}"/></td>
