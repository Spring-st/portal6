	...
	function updateRecordsByType(type){ 
			if(type!=""){
 			...
 			$.ajax({         
            	...
            	dataType: 'json',   //返回值类型        
            	success:function(date){
            		window.location.href="viewDifference.do?id=${x_wmsStocking.id}";
				} 
			})
		}
	}
...
ul li{display: inline;}
	...
	--%>
	<html:form action="/viewDifference.do" >
	<input type="hidden" name="fields" value="" desc="请选择"/>
	<input type="hidden" name="fields" value="sr.box.location.code" desc="系统库位"/>
	<input type="hidden" name="fields" value="sr.location.code" desc="实际库位"/>
	...
	<page:form action="/viewDifference.do">
	<input name="id" type="hidden" value="${x_wmsStocking.id}"/>
	<table width=100% border=0 cellpadding=4 cellspacing=0>
	<tr>
			<td colspan="2">
