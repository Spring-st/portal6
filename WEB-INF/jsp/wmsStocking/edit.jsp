...
<!--
	function itemview(locationCode, part) {
		v = window.showModalDialog(
			'showDialog.do?title=WmsStocking.edit.title&listWmsStockingItem.do?locationId='+locationCode+"&part="+part+"&stocking=${x_wmsStocking.id}", 
			...
			<td colspan="2">
				...
				<tbody id="datatable">	
					...
					<tr align="center" class="shares_${report.part.id}_${report.id}" id="shares_${report.part.id}">
						...
						<td>${report.location.code}
					   		<input style="display: none;" type="checkbox" checked="checked" name="ids" value="${report.id}"/>
						</td>
						...
						<td class="mxck" ><a href='javascript:itemview("${report.location.id}","${report.part.id}")'><bean:message key="wmsStocking.item.view" /></a></td>
					</tr>
