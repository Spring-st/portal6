...
<script type="text/javascript">
	function printCodeList(psoId) {
					...
					window.location.href="printPortalShipOrderPartCodeListWrong.do?supplierBatchStr="+supplierBatchStr+"&id="+psoId+"&poipBoxIds="+poipBoxIds;		
	...
	function printSPList(psoId) {
		window.location.href="printPortalShipOrderViewWrong.do?id="+psoId;
	}
	...
	function withdraw(psoId){
		var url = "withdrawPortalShipOrderTwoWrong.do?id=" + psoId;
		window.location.href = url;
	}
	function goTo(id,grantedSite){
		var url="";
		if(grantedSite==1){
			url = "listShippingOrderWrong.do";
		}else{
			url = "listShippingOrderWrong.do";
			//url = "shippingOrderWrongSupplier.do";
		}
		window.location.href = url;
	}
	function createPortalShipOrderIpTwo(psoId) {
		
		window.location.href="createPortalShipOrderIpTwoWrong.do?id="+psoId;
	}
	function closeEnabled(id){
		window.location.href="updateNPoCloseIsEnabled.do?id="+id+"&GrantedSite=1";
	}
	function openEnabled(id){
		window.location.href="updateNPoOpenIsEnabled.do?id="+id+"&GrantedSite=1";
	}
</script>
<html:form action="/viewPortalShipOrderTwoWrong" method="post">
	<table width="100%">
	<tr>
				...
				<tbody id="datatable2">
					<logic:iterate id="X_OBJECT" name="x_portalShipOrderItemList">
						...
						<tr  align="center">
							...
							<td>${X_OBJECT.portalShipOrder.supplier.code}</td>
							<td>${X_OBJECT.portalShipOrder.supplier.name}</td>
							<td>
								<fmt:formatNumber value="${X_OBJECT.deliveryNumber}" maxFractionDigits="0" minFractionDigits="0"/>
							</td>
							<td>
								<fmt:formatNumber value="${X_OBJECT.received_qty}" maxFractionDigits="0" minFractionDigits="0"/>
							</td>
							<td>
								<fmt:formatNumber value="${X_OBJECT.actual_qty}" maxFractionDigits="0" minFractionDigits="0"/>
							</td>
						</tr>
					</logic:iterate>
			    </tbody>
...
<page:form action="/viewPortalShipOrderTwoWrong">		
				...
				<c:set var="X_OBJECT"  scope="request" value="X_OBJECT"/>
					<tr align="center">	<td style="display: none"><input type="text" value="${sessionScope.size}" id="size"/></td>
							...
							<!-- 	<td id="supp" >
								...
								<td>${X_OBJECT.psoItem.part.id}</td>
								<td>
									<fmt:formatNumber value="${X_OBJECT.psoItem.part.ord_mult}" maxFractionDigits="0" minFractionDigits="0"/>
								</td>
								<td>
									<fmt:formatNumber value="${X_OBJECT.number}" maxFractionDigits="0" minFractionDigits="0"/>
								</td>
	...
	<table width=100% border=0 cellpadding=4 cellspacing=0 style="margin: 0px 0px 10px 0px;">
		<tr align="right">
			<td class="bluetext">合计数量:&nbsp;&nbsp;&nbsp;<fmt:formatNumber value="${sumNumber}" maxFractionDigits="0" minFractionDigits="0"/></td>
		</tr>
	</table>
	<table width="100%">
		<tr>
		...
		<tr>
			...
			<c:if test="${X_GRANTEDSITE==1}">
				<input type="button" value="返回" onclick="goTo('${x_portalShipOrder.id}',1)"/>
			</c:if>
			<c:if test="${X_GRANTEDSITE==2}">
				<input type="button" value="返回" onclick="goTo('${x_portalShipOrder.id}',2)"/>
			</c:if>
			<c:if test="${X_GRANTEDSITE==null}">
				<input type="button" value="返回" onclick="goTo('${x_portalShipOrder.id}',2)"/>
			</c:if>
			</td>
		</tr>
